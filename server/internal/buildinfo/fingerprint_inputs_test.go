...
//
//   - `bin/fd`      — 사용자 전원의 클라이언트(훅·MCP)
//   - `server/Dockerfile` — 공유 서버
//
...
// fpRepoRoot 는 이 시험이 읽는 레포 루트다(buildinfo 에서 세 단계 위).
//
...
func TestFingerprintInputsAgreeAcrossBuildSites(t *testing.T) {
	...
	launcher := fpRead(t, root, "bin/fd")
	docker := fpRead(t, root, "server/Dockerfile")
...
func TestFingerprintBothSitesHashFilesOnly(t *testing.T) {
	...
	if !fpLauncherFindRe.MatchString(fpRead(t, root, "bin/fd")) {
		t.Errorf("bin/fd 의 지문 계산이 `find . -type f \\( \"${src_inputs[@]}\" \\)` 모양이 아니다 — " +
			"`-type f` 가 빠지면 디렉토리가 해시 입력에 섞여 Dockerfile 쪽과 갈린다")
	}
	if !fpDockerFindRe.MatchString(fpRead(t, root, "server/Dockerfile")) {
...
func TestLauncherKeepsTheInputListInOnePlace(t *testing.T) {
	src := fpRead(t, fpRepoRoot(t), "bin/fd")
