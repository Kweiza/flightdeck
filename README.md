...
A coordination layer for parallel agent sessions. **Claude Code and codex sessions** register with
one server — many machines, many projects, many sessions.
...
> **Where this repository came from.** flightdeck grew inside the plugin collection
> [kweiza-cc-plugins](https://github.com/Kweiza/kweiza-cc-plugins) as `plugins/flightdeck`, and was split
> out here on 2026-09-24 at 0.38.3. **History was not carried over** — the earlier commits, and the
> commit shas that `DESIGN.md`, code comments and this document cite (`94fc82e` and so on), live in that
> repository's history. The "development repo" in the measurements below is that repository too, which
> is why the board above shows it as the project name.
...
This repository is itself the marketplace. Enable `flightdeck` from `/plugin`, or put it in your
settings directly.
...
> **Already installed as `flightdeck@kweiza-cc-plugins`? Leave it.** That marketplace's flightdeck
> entry points at this repository, so `/plugin update` fetches the same code. **Do not enable both** —
> you would get two sets of hooks and two MCP servers.

The client runs the same code as the server — `bin/fd` builds `server/` on first use, so **every machine
that runs sessions needs Go (1.25 or newer)** (without it you get a notice and the session carries on).
