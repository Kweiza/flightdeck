...
병렬 에이전트 세션의 조정 계층. **Claude Code 와 codex 세션**이 서버 하나에 붙는다 —
여러 머신, 여러 프로젝트, 여러 세션.
...
> **이 저장소의 출발점.** flightdeck 은 플러그인 모음 저장소
> [kweiza-cc-plugins](https://github.com/Kweiza/kweiza-cc-plugins) 의 `plugins/flightdeck` 에서 자랐고,
> 2026-09-24 에 0.38.3 을 기준으로 여기로 분리됐다. **이력은 가져오지 않았다** — 그전의 커밋과,
> `DESIGN.md`·코드 주석·이 문서가 인용하는 커밋 sha(`94fc82e` 등)는 그 저장소의 이력에 있다.
> 이 문서의 실측에 나오는 「개발 저장소」도 그 저장소다. 위 보드의 프로젝트명이 그것인 이유다.
...
이 저장소가 곧 마켓플레이스다. `/plugin` 에서 `flightdeck` 을 켜거나, 설정에 직접 넣는다.
...
> **이미 `flightdeck@kweiza-cc-plugins` 로 깔았다면 그대로 둬라.** 그 마켓플레이스의 flightdeck
> 항목은 이 저장소를 가리키므로 `/plugin update` 가 같은 코드를 받아 온다. **둘을 함께 켜지는
> 마라** — 훅과 MCP 서버가 두 벌 걸린다.

서버와 같은 코드를 쓴다 — `bin/fd` 가 첫 호출에 `server/` 를 빌드하므로 **세션을 돌리는
머신에도 Go(1.25 이상)가 있어야 한다**(없으면 아래처럼 안내만 내고 세션은 그대로 간다).
